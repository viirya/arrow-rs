...
impl JsonToVariantTest<'_> {
    ...
    fn run_without_validation(self) -> Result<(), ArrowError> {
        self.run_test(false)
    }

    fn run_test(self, validate: bool) -> Result<(), ArrowError> {
        let mut variant_builder = VariantBuilder::new();
        json_to_variant(self.json, &mut variant_builder)?;
        let (metadata, value) = variant_builder.finish();
        let variant = if validate {
...
#[test]
fn test_validate_json_to_variant_object_very_large() -> Result<(), ArrowError> {
    let json = get_very_large_json();
...
fn test_json_to_variant_object_very_large() -> Result<(), ArrowError> {
    ...
    json_to_variant(&json, &mut variant_builder)?;
    let (metadata, value) = variant_builder.finish();
    let v = parquet_variant::Variant::new(&metadata, &value);
